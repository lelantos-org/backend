...
pub struct QuoteRequest {
    ...
    /// Amount of `token_in` to swap, in the token's smallest unit.
    /// Decimal string to avoid JSON precision loss on amounts > 2^53.
    #[serde(with = "u256_dec")]
    #[schema(value_type = String, example = "1000000000")]
    pub amount_in: U256,
    /// User max slippage in basis points (e.g. 50 = 0.5%).
    pub slippage_bps: u16,
}

impl QuoteRequest {
    /// Apply the request's slippage tolerance to a quoter's `expected_out`,
    /// producing the floor used as `min_out` on-chain. `expected_out *
    /// (10_000 - slippage_bps) / 10_000`, computed with saturating ops so
    /// adversarial inputs cannot panic.
...
/// Best route found across racing quoters. Returned to SDK as JSON.
...
pub struct Quote {
    ...
    pub adapter: Address,
    /// Adapter-specific opaque blob; for UniV3 = `abi.encode(uint24 fee)`,
    /// or `abi.encodePacked(...)` for multi-hop. 0x-prefixed hex.
    ...
    /// Unix seconds at which the venue was queried. Frontend uses this to
    /// compute quote age / drive its own staleness UI.
    pub quoted_at: u64,
    /// MASP wrapper fee deducted from venue's gross output, in `token_out`
    /// smallest unit. Already applied to `expected_out` and `min_out`.
    #[serde(with = "u256_dec")]
...
/// Wire format for [`U256`] is decimal-string, mirroring how the relayer
/// SDK encodes shielded amounts. Keeps clients framework-agnostic.
mod u256_dec {
