...
crypto::test_clue(&detection_key, r_x, r_y, clue_bits, gamma);

// Hot path: parse the key once, test many clues against it.
let dk = crypto::filter::parse_detection_key(&detection_key, gamma)?;
crypto::filter::test_clue_parsed(&dk, r_x, r_y, clue_bits, gamma);
```
...
```sh
cargo bench -p crypto --bench clue_breakdown   # per-bit cost: scalar mul / Poseidon / Legendre
cargo bench -p crypto --bench filter_batch     # whole batched filter path
```
