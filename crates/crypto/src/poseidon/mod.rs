...
///
/// The arity is checked before the table is touched. [`hash`] is public and
/// takes any slice, so its length reaches here unvalidated; sizing the table by
/// it first would turn an unsupported arity into an allocation proportional to a
/// caller's input rather than the error `Circom::new` would have returned.
///
/// The table is then sized to `MAX_WIDTH` once rather than grown per arity: the
/// bound above is what makes that a fixed thirteen slots.
fn with_hasher<T>(
    arity: usize,
    f: impl FnOnce(&Circom) -> Result<T, PoseidonError>,
) -> Result<T, PoseidonError> {
    if !(1..MAX_WIDTH).contains(&arity) {
        return Err(PoseidonError::UnsupportedArity(arity));
    }
    HASHERS.with(|cell| {
        let mut slots = cell.borrow_mut();
        if slots.is_empty() {
            slots.resize_with(MAX_WIDTH, || None);
        }
