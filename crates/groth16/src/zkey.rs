...
pub struct ZkeyMatrices {
    ...
    /// Everything in the witness that is not an instance variable. The prover
    /// checks the witness against `num_instance_variables + num_witness_variables`,
    /// which is the zkey's `n_vars`.
    pub num_witness_variables: usize,
    pub num_constraints: usize,
    pub a: Matrix<Fr>,
    pub b: Matrix<Fr>,
    /// Always empty: snarkjs does not store C, and the snarkjs QAP reduction
    /// ([`crate::qap::CircomReduction`]) derives it from A and B. The prover
