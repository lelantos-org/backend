...
impl WitnessCalculator {
    ...
    pub fn new(graph_path: &Path) -> Groth16Result<Self> {
    ...
    pub fn calculate(&self, inputs: Inputs) -> Groth16Result<Vec<Fr>> {
        let json = inputs_to_json(&inputs)?;
        let wtns = calc_witness(&json, &self.graph)
            .map_err(|e| Groth16Error::Prove(format!("witness calculation failed: {e}")))?;
        parse_wtns(&wtns).map_err(|e| Groth16Error::Prove(format!("parse calculated witness: {e}")))
    }
}
...
fn inputs_to_json(inputs: &Inputs) -> Groth16Result<String> {
    let mut map = Map::with_capacity(inputs.len());
    for (name, values) in inputs {
        let mut out = Vec::with_capacity(values.len());
        for (index, value) in values.iter().enumerate() {
            if !in_range(value) {
                return Err(Groth16Error::Prove(format!(
    ...
    serde_json::to_string(&Value::Object(map))
        .map_err(|e| Groth16Error::Prove(format!("encode witness inputs: {e}")))
}

/// The scalar field modulus, as the integer type signal values arrive in.
fn field_modulus() -> &'static BigInt {
    static R: OnceLock<BigInt> = OnceLock::new();
    R.get_or_init(|| BigInt::from_bytes_be(Sign::Plus, &Fr::MODULUS.to_bytes_be()))
}

/// Whether a signal value is representable.
///
/// Rejects a negative or out-of-field value rather than letting it be wrapped
/// into a different signal value. The bound is the modulus rather than a byte
/// width: the graph reduces whatever it is given, so a value in `[r, 2^256)`
/// would yield a perfectly valid proof of a statement the caller did not ask
/// for, and `Groth16Params::verify` cannot catch that -- the reduced witness is
/// self-consistent.
fn in_range(value: &BigInt) -> bool {
    !value.is_negative() && value < field_modulus()
}
...
mod tests {
    ...
    /// A negative or out-of-field input must be refused rather than reduced into
    ...
    /// The values a byte-width check would wave through: `r` itself and the
    /// numbers just above it are 254 bits wide, so nothing about their size
    /// distinguishes them from a legitimate signal. The graph would reduce them
    /// and prove a different statement.
    #[test]
    fn values_at_or_above_the_modulus_are_rejected() {
        let r = field_modulus().clone();
        assert!(!in_range(&r), "the modulus reduces to zero");
        assert!(!in_range(&(&r + 1u8)));
        assert!(!in_range(&((BigInt::from(1u8) << 256) - 1u8)));
    }
