...
common_crypto::test_clue(&detection_key, r_x, r_y, clue_bits, gamma);

// Hot path: parse the key once, test many clues against it.
let dk = common_crypto::filter::parse_detection_key(&detection_key, gamma)?;
common_crypto::filter::test_clue_parsed(&dk, r_x, r_y, clue_bits, gamma);
```
...
`epk`'s cofactor is cleared rather than checked. Baby-Jubjub's group is
`Z_8 × Z_n`, so a sender can pick `epk = T + [t]B8` with `T` in the 8-torsion;
under a plain `[ivk]epk` exactly one of the eight choices decrypts, leaking
`ivk mod 8`. `try_decrypt` multiplies `[8]epk` by `ivk · 8^-1`, which is
`[ivk]epk` for an honest point and carries no torsion term otherwise, and
refuses a pure-torsion `epk` — whose shared secret is the identity for every
key.

`clue::unpack` validates on-curve only; `clue::unpack_subgroup` adds the
subgroup and non-identity checks, and is what address decoding uses. The clue
path keeps a full subgroup test of its own in `clue::usable_as_clue`.
...
```sh
cargo bench -p common-crypto --bench clue_breakdown   # per-bit cost: scalar mul / Poseidon / Legendre
cargo bench -p common-crypto --bench filter_batch     # whole batched filter path
```
