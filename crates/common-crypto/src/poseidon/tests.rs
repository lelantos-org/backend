...
/// `hash` is public and takes any slice, so an unsupported arity reaches
/// `with_hasher` as a length. It has to be refused before the hasher table is
/// sized by it, or a caller's slice length becomes an allocation.
#[test]
fn an_unsupported_arity_is_refused_without_sizing_the_table() {
    for arity in [0usize, MAX_WIDTH, MAX_WIDTH + 1, usize::MAX / 2] {
        let err = with_hasher(arity, |_| Ok(())).expect_err("arity out of range");
        assert!(
            matches!(err, PoseidonError::UnsupportedArity(a) if a == arity),
            "arity {arity}: got {err}"
        );
    }
}

/// And the supported ones still build, so the bound is not off by one at either
/// end: arity 1 is width 2 and arity `MAX_WIDTH - 1` is width `MAX_WIDTH`.
#[test]
fn every_supported_arity_still_builds() {
    for arity in ARITIES {
