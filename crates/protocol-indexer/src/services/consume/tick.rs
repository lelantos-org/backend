...
pub const OWNER: Owner = Owner::Protocol;
...
fn kinds() -> &'static [i16] {
    ...
    KINDS.get_or_init(|| {
        EventKind::kinds_for(Consumer::Protocol)
            .into_iter()
...
pub async fn tick_chain(ctx: &ConsumeCtx, chain_id: i64, batch: i64) -> Result<TickProgress> {
    ...
    if rows.is_empty() {
        ...
        // has room to retry.
        metadata::fill_missing(ctx, chain_id).await;
    ...
    // `RefreshGate`.
    if plan.touched_tree_advances() {
        ctx.refresh.mark(View::TreeAdvances).await;
    }
    ctx.refresh
    ...
    debug!(chain_id, processed = rows.len(), last_id, "protocol commit");
    Ok(progress)
}
...
/// returned flag is dropped — this loop holds the only writer per chain, so
/// `false` is unreachable rather than a condition to report.
