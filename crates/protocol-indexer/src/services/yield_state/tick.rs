...
pub const NAME: &str = "protocol-yield-state";
...
pub async fn tick_chain(ctx: &YieldStateCtx, chain_id: i64, _batch: i64) -> Result<TickProgress> {
    ...
    // A malformed column costs its own asset and nothing else; see
    // `domain::address::from_column` for why it is not decoded infallibly.
    let targets: Vec<(Address, u64)> = assets
        .iter()
        .filter_map(|a| match address::from_column(&a.venue) {
            Some(venue) => Some((venue, a.asset_id_u64 as u64)),
            None => {
                warn!(
                    chain_id,
                    asset_id = a.asset_id_u64,
                    "venue is not a 20-byte address"
                );
                None
            }
        })
    ...
    let round = reader.round(&targets).await?;

    for ((_, asset_id), state) in targets.iter().zip(round) {
        ...
        if let Err(e) = write_one(ctx, chain_id, asset_id, &state).await {
            warn!(chain_id, asset_id, error = %e, "yield state write failed");
        }
    }
