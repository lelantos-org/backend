...
fn llama_chain(chain: ChainId) -> Option<&'static str> {
    Some(match chain.get() {
        1 => "ethereum",
        10 => "optimism",
        137 => "polygon",
        8453 => "base",
...
/// is already lowercase and unprefixed, so this only re-dresses it.
fn coin_id(chain: &str, token_hex: &str) -> String {
    format!("{chain}:0x{token_hex}")
}
...
mod tests {
    ...
    fn llama() -> DefiLlama {
        DefiLlama::new("https://coins.llama.fi", Duration::from_secs(15)).unwrap()
    }

    #[test]
    fn local_chains_have_no_provider_slug() {
        // Keeps the anvil stack from calling out.
        assert_eq!(llama_chain(ChainId::new(31337)), None);
        assert_eq!(llama_chain(ChainId::new(1337)), None);
        assert_eq!(llama_chain(ChainId::new(1)), Some("ethereum"));
    }

    #[test]
    fn the_supported_chains_are_the_ones_with_a_slug() {
        // What the service filters on before it hands this provider a batch.
        let llama = llama();
        assert!(llama.supports_chain(ChainId::new(1)));
        assert!(!llama.supports_chain(ChainId::new(31337)));
    }

    #[test]
    fn a_coin_is_the_chain_slug_and_the_prefixed_address() {
        let k = key(1, "A0B8");
        assert_eq!(
            coin_id(llama_chain(k.chain).unwrap(), &k.address),
            "ethereum:0xa0b8"
        );
    }
    ...
    fn a_trailing_slash_on_the_base_url_does_not_double_up() {
        assert_eq!(
            DefiLlama::new("https://coins.llama.fi/", Duration::from_secs(1))
                .unwrap()
                .base_url,
    ...
    async fn the_live_provider_still_speaks_the_shape_we_parse() {
        ...
        let got = llama().fetch(std::slice::from_ref(&usdc)).await.unwrap();
