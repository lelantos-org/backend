...
sol! {
    ...
    event DepositEscrowed(
        ...
        // than by filtering. `feeAssetId` is the asset the fee note is minted
    ...
    /// when it was paid in the same asset). `feeRefunded` is the relayer fee
    /// refunded separately in `feeAssetId`'s token, nonzero only when the fee
    /// was paid in a different asset than the deposit.
    #[derive(Debug)]
