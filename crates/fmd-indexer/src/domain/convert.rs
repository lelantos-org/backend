...
/// scale 0, so the `BigDecimal` is a plain integer. A non-zero scale would
/// silently yield the wrong element, which is what the unit test below pins.
...
///
/// Big-endian, matching what the contract writes. `fmd_crypto::filter`
/// documents its `clue_bits` argument as little-endian, which describes the
/// *bit* order inside the u16, not the byte order on the wire — the two are
/// consistent and `tests/fixture_replay.rs` pins the round trip.
...
mod tests {
    ...
    fn bigdec_to_fq_reads_scale_zero_integers() {
        // The shape every `NUMERIC(78, 0)` read takes. If diesel ever hands
        // back a scaled representation of the same value (1E+2 for 100), the
        // exponent this drops would change the answer.
