...
//!
//! Both endpoints spend shielded notes and differ only in which pipeline
//! executes the payload. Everything around that — admission control, the
//! replay window, the fingerprint pinning a key to its payload — is identical,
//! and the *order* of those steps is load-bearing, so it lives here once
...
///
/// 1. Nullifiers and fingerprint are parsed *before* the idempotency run, so a
///    malformed payload is a 400 rather than something cached against a key.
/// 2. The nullifier reservation happens *inside* it, because a resubmission
///    under a known key must replay the first answer — reserving outside would
///    refuse it as a double-spend before it ever got there.
/// 3. The reservation is only marked spent after the transaction lands, which
///    holds it against a resubmit until the indexer catches up.
...
/// What makes this submission *this* submission.
///
/// The idempotency key is chosen by the caller and carries no proof of what it
/// stands for. Storing this alongside the answer lets a key reused with
/// different contents be refused, instead of silently swallowing the second
/// submission and handing back the first one's transaction hash.
///
/// Nullifiers and output commitments are enough: two payloads that agree on
/// both spend the same notes into the same notes.
///
/// Both are hashed as their *parsed* 32-byte values, never as the wire strings.
...
/// submission and defeat the replay protection it exists to provide.
