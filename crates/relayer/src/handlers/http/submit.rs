...
use crate::services::admission::nullifier_guard::{Nullifiers, PendingGuard, nullifiers_of};
...
/// 3. The reservation goes to the pipeline with the operation. The batcher marks
///    it spent once the bundle lands, holding it against a resubmit until the
///    indexer catches up, and holds it until then even if this caller hangs up.
/// 4. The run is spawned, so a caller that hangs up does not cancel it. The
///    handler is dropped when the client gives up or the router's timeout fires,
///    and with it any future it was polling; were the idempotency run one of
///    them, the key's entry would be dropped mid-flight and the wallet's retry
///    would find only the nullifier reservation, a 409. Spawned, the run stays in
///    flight and the retry waits on it for its transaction hash.
async fn submit<P, Run, Fut>(
    st: AppState,
    headers: HeaderMap,
    payload: P,
    run: Run,
...
where
    P: Submission + Send + 'static,
    Run: FnOnce(P, PendingGuard) -> Fut + Send + 'static,
    Fut: Future<Output = AppResult<SubmissionReceipt>> + Send + 'static,
{
    ...
    let tx_hash = tokio::spawn(async move {
        st.idempotency
            .run(chain_id, key, fingerprint, async {
                ...
                // whole state machine on the task's frame.
                let receipt = Box::pin(run(payload, reservation)).await?;
                Ok(format!("0x{}", hex::encode(receipt.tx_hash)))
            })
            .await
    })
    .await
    .map_err(|e| AppError::Internal(format!("submission task: {e}")))??;
...
fn submission_fingerprint(nfs: &Nullifiers, pi: &PubInputsDto) -> AppResult<[u8; 32]> {
