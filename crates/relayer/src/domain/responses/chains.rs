...
/// are protocol-webserver's `/v1/chains` and `/v1/assets`, identical for every
...
pub struct ChainHealth {
    ...
    pub desynced: bool,
    /// EIP-55 checksummed address of this relayer's `Bundler`, the contract that
    /// calls the pool and the swap wrapper. Wallets bind it into the SNARK as
    /// `relayer` (spends) or `payer` (swaps), and the contracts reject a proof
    /// naming anyone else.
    pub relayer_address: String,
    /// EIP-55 checksummed account this relayer offers as a swap's
    /// `swap.refundTo` for wallets with no EVM account of their own. Unlike
    /// `relayerAddress` it can move tokens, so a cancelled output escrow refunded
    /// here is recoverable (by the operator). Defaults to the signer's address;
    /// see `refund_address` in the relayer config.
    ...
    /// protocol-webserver publishes the depth the *deployment* declares, this
...
/// to `/v1/spend/estimate`, for the same reason protocol-webserver publishes spot
...
pub struct ShieldedFeeOut {
    ...
    /// deployment catalog protocol-webserver publishes.
...
impl TokenOut {
    ...
    /// protocol-webserver, on the catalog it owns. A relayer restating it would be
...
mod tests {
    ...
    /// The deployment half moved to protocol-webserver. A relayer that still
    ...
    fn test_serialize_chain_health_publishes_nothing_about_the_deployment() {
        ...
        for moved in [
            ...
            assert!(
                ...
                "{moved} belongs to protocol-webserver /v1/chains"
            );
        }
    }
    ...
    fn test_serialize_chain_health_carries_every_relayer_reading() {
        ...
        assert_eq!(json["refundAddress"], "0xREFUND");
