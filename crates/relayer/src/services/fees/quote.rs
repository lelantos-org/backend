...
use crate::services::fees::gas_estimator::{GasEstimator, apply_markup};
use crate::services::fees::oracle::PriceOracle;
...
impl FeeQuoter {
    ...
    fn quote_one(
        ...
        Some(FeeQuote {
            ...
            // service does not depend on. See `pipeline::transact::FeeContext::quote`.
            asset_id: None,
            scale: None,
            circuit_amount: None,
        })
    ...
    pub async fn quote_for_gas(&self, gas_used: u64) -> AppResult<EstimateResponse> {
        let gas_price_wei = self.gas_estimator.effective_gas_price_wei().await?;
        let total_native_wei = apply_markup(gas_used, gas_price_wei, self.markup_bps);
        ...
        Ok(EstimateResponse {
            gas_used,
            effective_gas_price_wei: gas_price_wei.to_string(),
