...
///
/// Each field is updated independently, which is enough for a display
/// endpoint: a reader that catches a mutation mid-flight sees a root one
/// advance away from the count, no worse than the staleness it already has
/// by not holding the lock.
...
impl MirrorSnapshot {
    ...
    /// Publish a fresh set of readings. Only [`super::TreeMirror`] calls
    /// this, and only while holding the mirror itself.
    pub(super) fn publish(&self, leaf_count: u64, root: Option<Field>, desynced: bool) {
        ...
        // A root that cannot be computed leaves the last good one in place
        // rather than publishing zero; the mirror is on its way to being
        // parked in that case anyway.
