...
impl DepositFailures {
    ...
    /// Test-only, like [`Self::deferred_len`]: production reads exclusions
    /// through [`Self::excluded_ids`].
    #[cfg(test)]
    fn quarantined_ids(&self) -> Vec<u64> {
...
/// and `MirrorDesynced` park the mirror, which the batcher resyncs on its own.
///
