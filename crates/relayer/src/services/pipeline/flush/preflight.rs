...
pub enum FeeGate {
    ...
    /// and worth at least `required` circuit units of
    ...
    /// would flush.
    Subsidised,
    /// The fee could not be priced: a fee asset this relayer will not take, or an
...
mod tests {
    ...
    /// Cross-asset: its fee note pays in another asset than the deposit's.
    ...
    /// A zero-fee deposit (fee asset 0) still hashes to its slot, and on a
    /// charging relayer its worthless note either is not ours or pays nothing, so
    /// it defers. `fee_gate` reaches this verdict rather than `Unpriceable`
    /// because [`PendingDeposit::fee_pricing_asset`] prices it in the deposit's
    /// asset.
    #[test]
    fn a_zero_fee_deposit_on_a_charging_relayer_is_deferred() {
        let d = PendingDeposit {
            fee_asset_id: 0,
            fee_in: 0,
            ..deposit()
        };
        for note in [FeeNote::NotOurs, FeeNote::Paid { paid: 0 }] {
            let gate = FeeGate::Charged {
                note,
                required: 250,
            };
