...
/// The row this query yields. Defined in `domain`, which is where the digest
/// derivation and the pre-flight table read it; re-exported so a caller of the
/// mempool still finds it here.
pub use crate::domain::deposit::{EscrowLeaf, PendingDeposit};
