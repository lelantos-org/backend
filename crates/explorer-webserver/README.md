...
| `/v1/yield` | `chainId?` | Yield-bearing assets, in registry order. An asset is yield-bearing **iff** it has an `asset_yield` row — `YieldAssetAdded` creates the binding and no event undoes it — so a halted asset is still listed, and `/v1/assets` carries no yield flag to disagree with this. The row has two halves that fail apart: `venueHex`, `bufferBps`, `perfBps` and `halted` are event-sourced and always present, while `gross`, `idle`, `accruedFee`, the normalized pair and `indexRay` are polled from `yieldState` and are **`null` together** until the first poll lands — `updatedAt` ages what is present. **Two units, and mixing them is the trap**: `gross`/`idle`/`accruedFee` are whole tokens (`null` when decimals are unresolved), while `totalNormalized`/`accruedFeeNormalized` are normalized share units and must never be rendered as an amount of the asset. `gross` is the balance, and is what `/v1/locked` reports for this asset under `venueHoldings`. `accruedFee` is computed as `accrued * gross / supply` in integers, the contract's own conversion — **not** rebuilt from `indexRay`, which is that ratio already rounded to 27 places and disagrees at the boundary; `indexRay` is a decimal string for display and charts only. A zero supply reports `accruedFee: null`, not `0`: there is no rate to convert at, which is not the same as owing nothing. |
